@app.route('/proctor/getNameId',methods=["GET", "POST"])
def proctor_get_name_id():
    ...
    if request.method == 'POST':
        ...
        if db_response != None:
            ...
            response_string = reset_proctor_password(email,name,regId)
            return response_string
    return json.dumps({"status": "user not found","response": "User not found"})



def reset_proctor_password(email,name,regId):
    message = MIMEMultipart()
    part1 = MIMEText('Dear '+name+" !\n"+"A password reset has been requested for proctor ID "+regId+".\nPlease click the below link to verify your identity and set a new password for your account.\n",'plain')
    part2 = MIMEText(u'<a href="http://180.179.48.5:80/proctor/forgot-password">Click here to reset your password</a>','html')
...
#PROCTOR FORGOT  PASSWORD
@app.route('/proctor/forgot-password',methods=["POST"])
def proctor_forgot_password():
    db = myclient["accounts-db"]
    collection = db["teacher"]
    if request.method == 'POST':
        ...
        if db_response != None:
            ...
            if update_result.acknowledged:
                ...
                if db_res!=None:
                    response_string = update_proctor_password(db_res["email"],db_res["regId"],db_res["password"],db_res["name"])
                    return response_string
...
def update_proctor_password(email,regno,password,name):
    message = MIMEMultipart()
    part1 = MIMEText('Dear '+name+" !\nYour password is successfully reset\nYour ID: "+regno+ '\n Password: '+password+'\n','plain')
    part2 = MIMEText(u'<a href="http://180.179.48.5:80/proctor/login">Click here to login</a>','html')
    part3 = MIMEText('\n\nWarm Regards and thanks,\nJakstech Team,\nJakstech Solutions.','plain')
    message.attach(part1)
    message.attach(part2)
    message.attach(part3)
